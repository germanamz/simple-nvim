    ...
    .new({}, {
      ...
      finder = finders.new_table({
        ...
        entry_maker = function(line)
          ...
          local icon, hl
          if staged[line] then
            icon, hl = "◆ ", "SmartFilesStaged"
          elseif modified[line] then
            icon, hl = "● ", "SmartFilesModified"
          elseif untracked[line] then
            icon, hl = "○ ", "SmartFilesUntracked"
          elseif committed[line] then
            icon, hl = "◈ ", "SmartFilesCommitted"
          end
          if icon then
            ...
            e.display = function(entry)
              ...
              if type(base) == "function" then
                d, base_hl = base(entry)
              else
                d = base or entry.value
              end
              local text = icon .. d
              local hls = { { { 0, #icon }, hl } }
              if base_hl then
