...
- **Long lines:** soft-wrap is off, so wide lines are cut at the right edge. Rest the cursor on one for a couple of seconds and it **unfolds in place** — wrapped across the rows below, which are pushed down rather than covered — then folds back the moment you move. Automatic; there is no key for it and nothing to dismiss.
